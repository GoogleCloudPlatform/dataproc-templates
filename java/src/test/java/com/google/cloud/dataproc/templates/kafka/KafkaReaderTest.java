...
class KafkaReaderTest  {
  ...
  static private Dataset<Row> jsonData;
  static private Dataset<Row> bytesData;
  ...
  @BeforeAll
   static void  setUp() throws IOException {
    ...
    //Dataframe Schema
    List<org.apache.spark.sql.types.StructField> listOfStructField = Arrays.asList(
  ...
  void runProcessJsonMessageWithValidParameters() {
    //Mock message schema
    messageSchema = DataTypes.createStructType(Arrays.asList(
        DataTypes.createStructField("id", DataTypes.StringType, true),
        DataTypes.createStructField("val", DataTypes.StringType, true)));

    reader = new KafkaReader();
    assertEquals(org.apache.spark.sql.Dataset.class,
        reader.processJsonMessage(jsonData, messageSchema).getClass());

    assertEquals("001", reader.processJsonMessage(jsonData,messageSchema)
        .select("id").collectAsList().get(0).get(0).toString() );
  ...
  void runProcessJsonMessageWithoutSchema(){
    ...
    messageSchema = null;
    assertThrows(NullPointerException.class,()->reader.processJsonMessage(jsonData,messageSchema));
  }
  ...
  void runProcessBytestMessageWithValidParameter(){
    ...
    messageSchema = null;
    assertEquals(org.apache.spark.sql.Dataset.class,
        reader.processBytesMessage(bytesData).getClass());

    assertEquals("value",reader.processBytesMessage(bytesData)
        .select("value").collectAsList().get(0).get(0).toString() );
  }
