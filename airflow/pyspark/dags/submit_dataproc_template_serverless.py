...
config = read_configuration("config/submit_dataproc_template_serverless.ini")
...
with models.DAG(
    ...
    schedule_interval=config['COMPOSER']['SCHEDULE_INTERVAL'],
    catchup=False,
    ...
    run_dataproc_template = BashOperator(
        ...
        bash_command="cd ./dependencies/dataproc_templates/python && bin/start.sh -- " + template_args,
        env=dict(config.items('ENV_VARS')),
        cwd=dag.folder
    )
